"""HuizenZoeker.nl scraper - aggregator pulling from many small agencies."""
...
class HuizenZoekerScraper(BaseScraper):
    name = "huizenzoeker"
    base_url = "https://www.huizenzoeker.nl"
    ...
    def scrape(self, filters: SearchFilter) -> list[Listing]:
        ...
        try:
            ...
            items = soup.select('[class*="listing"]') or soup.select('[class*="property"]')
            for item in items:
            ...
            logger.error(f"[huizenzoeker] Error scraping: {e}")

        logger.info(f"[huizenzoeker] Found {len(listings)} listings")
        return listings
    ...
    def _parse_item(self, item) -> Listing | None:
        try:
            ...
            logger.debug(f"[huizenzoeker] Parse error: {e}")
            return None
